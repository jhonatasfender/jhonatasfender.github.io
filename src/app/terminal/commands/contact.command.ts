...
export class ContactCommand extends BaseTerminalCommand {
  ...
  public execute(): void {
    ...
    this.terminal.addLine('');
    this.terminal.addLine('💡 Dica: Clique nos links para abrir em uma nova aba');
    this.terminal.addLine('');

    const githubLink = this.translateService.instant('CONTACT.GITHUB').toString().split(': ')[1];
    const linkedinLink = this.translateService.instant('CONTACT.LINKEDIN').toString().split(': ')[1];

    this.terminal.addLine(`🔗 GitHub: <a href="${githubLink}" target="_blank">${githubLink}</a>`);
    this.terminal.addLine(`🔗 LinkedIn: <a href="${linkedinLink}" target="_blank">${linkedinLink}</a>`);
  }
