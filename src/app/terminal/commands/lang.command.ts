  ...
  public execute(): void {
    ...
    this.terminal.addLine(
      this.translateService
        .instant('LANGUAGE.CURRENT', { lang: currentLang })
        .toString(),
    );
    this.terminal.addLine('');

    this.terminal.addLine(
      this.translateService.instant('LANGUAGE.AVAILABLE').toString(),
    );
    this.terminal.addLine('='.repeat(30));

    availableLangs.forEach((lang: string) => {
      const langName = this.translateService.instant(
    ...
    if (this.validLanguages.includes(normalizedLang)) {
      ...
      this.translateService.use(normalizedLang).subscribe({
        next: () => {
          this.terminal.addLine(
            this.translateService
              .instant('LANGUAGE.CHANGED', { lang: normalizedLang })
              .toString(),
          );
        },
        error: () => {
          this.terminal.addLine(
            this.translateService.instant('LANGUAGE.ERROR').toString(),
          );
        },
      });
    } else {
      this.terminal.addLine(
        this.translateService.instant('LANGUAGE.INVALID').toString(),
      );
      this.execute();
    }
  }
